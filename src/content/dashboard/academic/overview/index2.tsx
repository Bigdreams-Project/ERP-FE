...
const OverviewContent = ({
  ...
  // compute derived datasets when range or input records change
  useEffect(() => {
    ...
    // Funnel: example steps - Inquiries -> Followups Scheduled -> Converted (students)
    // We consider "follow-up scheduled" as leads with nextFollowUpDate in range
    const followUpScheduled = leadsInRange.filter((l) =>
      l.nextFollowUpDate ? includesDate(l.nextFollowUpDate, start, end) : false
    ).length;

    // Conversions: count students whose leadId is one of leadsInRange ids
    ...
    // Funnel data for chart (keeps order)
    const newFunnel = [
      { name: "Inquiries", value: totalLeads },
    ...
    // Academic summary cards (example): centers, faculties, students, lead->enroll ratio
    const newAcademicStats = [
      ...
      },
      {
        title: "Leads (range)",
        value: formatNumber(totalLeads),
        subtitle: `${percent(totalLeads, Math.max(1, leads.length))} of all`,
      },
      {
        title: "Conversion",
        value: `${conversionRate.toFixed(1)}%`,
        subtitle: `${convertedFromLeadsInRange} direct conversions`,
      },
    ];

    // Recent activity: merge leads and student enrollments within range, sort by created/enquiry/enrolled date
    // Use enquiryDate for leads, enrolledDate for students
    ...
    // Build insights list (simple examples)
    const newInsights = [
      {
        icon: "📈",
      ...
      { icon: "🎓", text: `There were ${totalStudents} enrollments.` },
      { icon: "🔁", text: `Conversion rate: ${conversionRate.toFixed(1)}%` },
      {
        icon: "🏆",
  ...
  // default UI: show initial displayRange on first render
  useEffect(() => {
    const start = range[0].startDate!;
    const end = range[0].endDate!;
    setDisplayRange(
        ...
        <div className="overview-gradient p-6 rounded-xl shadow-sm shadow-gray-400 mb-6 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-300 mt-1">
              Here's what's happening in the selected date range.
            </p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm shadow-gray-400 relative">
            ...
            <ul className="space-y-4 mt-4">
              {insights.map((insight, index) => (
                ...
                >
                  <span className="w-6">{insight.icon}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {courseStats.map((stat: any, index: number) => (
              <AcademicStatCard
                key={index}
                title={stat.title}
                value={`${stat.leads} leads`}
                subText={`Enrolls: ${stat.enrolls} • Conv: ${stat.conversion}`}
                icon={""}
              />
            ))}
          </div>
