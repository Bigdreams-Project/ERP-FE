  ...
  return (
    <div className="flex space-x-1 font-medium mt-6 items-center">
      ...
      <button
        ...
        disabled={isOverview}
        className={`text-indigo-600 hover:text-indigo-800 mr-2 ${isOverview ? 'opacity-50 cursor-not-allowed' : ''}`}
      >
      ...
      {paths.map((path, index) => (
        <div key={index} className="flex items-center space-x-1 text-[17px] font-inter">
          
          <span className="text-[rgba(0,0,0,0.7)]"><FiChevronRight size={16} /></span>
          {path.href ? (
            <Link href={path.href}>
              <span className="text-indigo-500 hover:underline cursor-pointer">{path.name}</span>
            </Link>
          ) : (
